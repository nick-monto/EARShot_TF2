...
base_Dir <- 'D:/Python_Programming/EARShot_TF2/Test/'
talker_List <- c('Victoria')
epoch_with_Exclusion <- 1500
epoch_without_Exclusion <- 2000
max_Display_Step <- 60
hidden_Type <- 'LSTM'
hidden_Unit <- 512
index <- 0
...
{
  work_Dir <- base_Dir
  rt_Data <- read_delim(paste(work_Dir, 'RTs.txt', sep=""), "\t", escape_double = FALSE, locale = locale(encoding = "UTF-8"), trim_ws = TRUE)
  ...
  {
    categorized_Flow_Data <- read_delim(paste(work_Dir, sprintf('Categorized_Flow/Categorized_Flows.txt', epoch), sep=""), "\t", escape_double = FALSE, locale = locale(encoding = "UTF-8"), trim_ws = TRUE)
